...
public:
    Trie() {
        root = new Node();
        root->freq = -1; //Initialize root's freq with -1
    }
    void insert(string key) {
        ...
        for (int i = 0; i < key.size(); i++) {
            if (temp->children.count(key[i]) == 0) { // Does not exist
                ...
                temp->children[key[i]]->freq = 1; //Set new node's freq to 1 as it occurred for the 1st time
            }
            else{
                temp->children[key[i]]->freq++; //Increment the freq if node already exists in Trie
                }
