...
void preOrder(Node* root) //Preorder: Root -> Left subtree -> Right subtree
{
    if(root==NULL)
        return;
    cout<<root->data<<" ";
...
void inOrder(Node* root) //Inorder: Left subtree -> Root -> Right subtree
{
    if(root==NULL)
        return;
    inOrder(root->left);
...
void postOrder(Node* root) //PostOrder: Left subtree -> Right subtree -> Root
{
    if(root==NULL)
        return;
    postOrder(root->left);
...
void levelOrder(Node* root) //BFS with new line character at the end of each level by using NULL 
{
    if(root==NULL)
        return;
    queue<Node*> Q;
